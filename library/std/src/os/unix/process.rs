...
///
/// Note that Fuchsia diverges from other Unix target behavior. There is a
/// discussion tracking the possibility of [removing Fuchsia's Unix target
/// family](https://github.com/rust-lang/rust/issues/58590).
///
...
pub impl(self) trait ExitStatusExt {
    ...
    /// # Example
    ///
    /// A signal-terminated [`wait`] status carries the signal number. [`ExitStatus::signal`]
    /// recovers it with the platform's [`WTERMSIG`][`wait`] macro. The (unspecified) bit layout
    /// conventionally occupies the low 7 bits with the exit-code byte left zero, so a bare
    /// signal number between 1-126 is a valid signal-terminated wait status:
    ///
    /// ```
    ...
    ///
    /// Rust does not invoke commands with a shell, such as `bash` and `sh`, by default, but it
